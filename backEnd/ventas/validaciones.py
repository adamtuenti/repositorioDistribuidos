...
def validate_fecha(fecha_inicio,fecha_fin,name_fecha_inicio='de inicio',name_fecha_fin='de fin'):
    i = str(fecha_inicio)
    f = str(fecha_fin)
    valores_inicio = i.split("-")
    valores_fin = f.split("-")
    if (valores_fin[0] < valores_inicio[0]):
        raise forms.ValidationError("La fecha {} es menor que la fecha {}".format(name_fecha_fin,name_fecha_inicio))
...
def validate_date_with_offset(start_date,end_date,offset):
    new_date = start_date-timedelta(days=offset)
    if end_date < new_date:
        raise forms.ValidationError("La fecha deber ser máximo {} días antes del registro".format(offset))
    return end_date